...
$required = ['firstname','middlename','lastname','bdate','contact','address','email','office','role','doctype','docnum','username','password'];
...
foreach ($handlesD as $chD) {
    ...
    if ($dh>=200 && $dh<300) {
        ...
        if (is_array($arr) && count($arr)>0) { curl_multi_remove_handle($mhD, $chD); curl_close($chD); curl_multi_close($mhD); respond(409, 'An admin with the same first and last name already exists.'); }
    }
...
$ANON_KEY = getenv('SUPABASE_ANON_KEY')
    ...
    ?: loadEnvValue('SUPABASE_PUBLIC_ANON_KEY')
    ?: getenv('SUPABASE_KEY')
    ?: loadEnvValue('SUPABASE_KEY')
    ?: (isset($_ENV['SUPABASE_ANON_KEY']) ? $_ENV['SUPABASE_ANON_KEY'] : null)
    ?: (isset($_ENV['SUPABASE_PUBLIC_ANON_KEY']) ? $_ENV['SUPABASE_PUBLIC_ANON_KEY'] : null)
    ?: (isset($_ENV['SUPABASE_KEY']) ? $_ENV['SUPABASE_KEY'] : null);
if (!$ANON_KEY) {
    $maybeLib = __DIR__ . '/../../../lib/supabase_client.php';
    if (is_file($maybeLib)) {
        require_once $maybeLib;
        if (function_exists('sb_anon_key')) {
...
$ch = curl_init();
curl_setopt($ch, CURLOPT_URL, rtrim($SUPABASE_URL,'/').'/rest/v1/reviewadmin');
