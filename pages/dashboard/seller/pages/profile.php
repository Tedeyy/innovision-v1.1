...
if ($pdo && $userId) {
  if ($_SERVER['REQUEST_METHOD']==='POST') {
    ...
    if ($updates) {
      try {
        ...
        $pdo->prepare('UPDATE sellers SET '.implode(',', $updates).' WHERE user_id = :user_id')->execute($params);
  ...
  try {
    $stmt=$pdo->prepare('SELECT '.implode(',', $columns).' FROM sellers WHERE user_id = :user_id');
